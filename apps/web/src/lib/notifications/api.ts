/**
 * REST client for the Vaasenk notifications surface — Sprint 6.2.
 *
 * Backed by the shared {@link apiFetch} / {@link apiFetchEnvelope} helpers,
 * which:
 *   • Attach the Supabase access token via the browser supabase client.
 *   • Unwrap the standard `{ data }` envelope (CLAUDE.md §5).
 *   • Throw {@link ApiClientError} on any non-2xx response, populated with
 *     the API's `{ error.code, error.message }` envelope.
 *
 * `apiFetchEnvelope` is used for the list endpoint specifically because we
 * need `meta.unreadTotal` — the standard helper drops `meta`.
 */
...
export async function listNotifications(
  ...
  // The backend's service returns `{ data, meta }` directly which the
  // ResponseInterceptor passes through. `meta` will always include
  // `unreadTotal` per the gateway contract.
  return {
    data: envelope.data ?? [],
    meta: (envelope.meta ?? {
      page: 1,
 ...
 *
 * Returns the number of rows updated — useful for a "32 notifications
